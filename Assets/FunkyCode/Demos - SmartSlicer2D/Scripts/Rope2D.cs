...
public class Rope2D : MonoBehaviour {
	...
	void Start () {
		...
		while (Vector2D.Distance (position, new Vector2D(connectedBody.transform.position)) > distance) {
			float direction = Vector2D.Atan2 (new Vector2D(connectedBody.transform.position), position);
	...
	public void OnRenderObject() {
		...
		foreach(HingeJoint2D joint in nodes) {
			...
			GL.TexCoord2(0, 0);
			GL.Vertex3(pairA.A.vector.x, pairA.A.vector.y, z);
			GL.TexCoord2(1, 0);
			GL.Vertex3(pairA.B.vector.x, pairA.B.vector.y, z);
			GL.TexCoord2(1, 1);
			GL.Vertex3(pairB.A.vector.x, pairB.A.vector.y, z);
			GL.TexCoord2(0,1);
			GL.Vertex3(pairB.B.vector.x, pairB.B.vector.y, z);
	...
	Pair2D GetPair(Vector2 pA, Vector2 pB) {
		...
		float rot = Vector2D.Atan2(pA, pB);
