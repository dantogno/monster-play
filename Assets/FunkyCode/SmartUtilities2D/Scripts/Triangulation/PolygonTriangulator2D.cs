		...
		switch (triangulation) {
			case Triangulation.Advanced:
				...
				foreach(Pair2D pair in Pair2D.GetList(polygon.pointsList)) {
					Vector3 pointA = new Vector3(pair.A.vector.x, pair.A.vector.y, 0);
					Vector3 pointB = new Vector3(pair.B.vector.x, pair.B.vector.y, 0);
					Vector3 pointC = new Vector3(pair.B.vector.x, pair.B.vector.y, 1);
					Vector3 pointD = new Vector3(pair.A.vector.x, pair.A.vector.y, 1);
