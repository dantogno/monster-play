...
public class Max2D {
	...
	{
		Vector2D p0 = new Vector2D (p.vector.x - size, p.vector.y - size);
		Vector2D p1 = new Vector2D (p.vector.x + size, p.vector.y - size);
		Vector2D p2 = new Vector2D (p.vector.x + size, p.vector.y + size);
		Vector2D p3 = new Vector2D (p.vector.x - size, p.vector.y + size);
	...
	static public void DrawStrippedLine(List<Vector2D> pointsList, float minVertsDistance, float z = 0f, bool full = false, Vector2D offset = null)
	{
		...
		if (setBorder == true) {
			...
			foreach (Pair2D id in Pair2D.GetList(pointsList, full)) {
				vA = new Vector2D (id.A.vector + offset.vector);
				vB = new Vector2D (id.B.vector + offset.vector);
