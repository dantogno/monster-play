...
public class ComplexSlicerExtended {
	...
	static public Polygon2D CreateSlice(List<Vector2D> slice) 
	{
		if (slice.Count () > 2 && Math2D.SliceIntersectItself(slice) == false)
	...
	static public Slice2D Slice(Polygon2D polygon, Polygon2D polygonSlice)
	{
		...
		if (startPoint == null) {
			Slicer2D.Debug.LogError ("Starting Point Error In PolygonSlice");
			return(result);
		}
		...
		*/
		...
		if (result.polygons.Count < 1) {
			Slicer2D.Debug.LogError ("Returns Empty Polygon Slice");
		}
	...
	static public Slice2D ComplexCutSlice(Polygon2D polygon, ComplexCut complexCut) {
		...
		if (Math2D.SliceIntersectItself(slice)) {
			Slicer2D.Debug.LogError("Complex Cut Slicer intersected with itself!");
			return(result);
		}
